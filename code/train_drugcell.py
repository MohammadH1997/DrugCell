...
def train_model(root, term_size_map, term_direct_gene_map, dG, train_data, gene_dim, drug_dim, model_save_folder,
    ...
    for epoch in range(train_epochs):
        ...
        mse = torch.mean((train_predict.cpu() - train_label_gpu.cpu()) ** 2)
        print("train mse  : ", mse)
        ...
        mse = torch.mean((test_predict.cpu() - test_label_gpu.cpu()) ** 2)
        print("test mse : ", mse)
...
def old_main():
    ...
    train_model(root, term_size_map, term_direct_gene_map, dG, train_data, num_genes, drug_dim, opt.modeldir, opt.epoch,
                opt.batchsize, opt.lr, num_hiddens_genotype, num_hiddens_drug, num_hiddens_final, cell_features,
                drug_features)
...
def main():
    ...
    onto = data_dir / 'drugcell_ont.txt'

    gene2id = data_dir / 'gene2ind.txt'
    gene2id_mapping = load_mapping(gene2id)
    num_genes = len(gene2id_mapping)

    train = data_dir / f'drugcell_train{postfix}.txt'
    test = data_dir / f'drugcell_val{postfix}.txt'
    cell2id = data_dir / f'cell2ind{postfix}.txt'
    drug2id = data_dir / 'drug2ind.txt'
    fingerprint = data_dir / 'drug2fingerprint.txt'
    ...
    lr = 0.001
    num_hiddens_drug = list(map(int, '100,50,6'.split(',')))
    num_hiddens_final = 6

    genotype = data_dir / 'cell2mutation.txt'

    alpha = 1

    cell_features = alpha * (np.genfromtxt(genotype, delimiter=','))

    train_model(root, term_size_map, term_direct_gene_map, dG, train_data, num_genes, drug_dim, modeldir, epoch,
