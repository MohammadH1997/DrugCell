...
def predict_dcell(predict_data, gene_dim, drug_dim, model_file, hidden_folder, batch_size, result_file, cell_features,
                  drug_features):
    ...
    for i, (inputdata, labels) in enumerate(test_loader):
        ...
        print(torch.cat([labels.cpu(), aux_out_map['final'].cpu()], dim=1).detach().numpy())
    ...
    mse = torch.mean((test_predict.cpu() - predict_label_gpu.cpu()) ** 2)
    print("predict_drugcell mse : ", mse)
...
def old_main():
    ...
    predict_dcell(predict_data, num_genes, drug_dim, opt.load, opt.hidden, opt.batchsize, opt.result, cell_features,
                  drug_features)
...
def main():
    ...
    model_dir = current_dir / '../pretrained_model'
    model_file = model_dir / 'drugcell_v1.pt'
    load = model_file

    predict = datadir / 'drugcell_test_2.txt'
    cell2id = datadir / 'cell2ind_2.txt'
    drug2id = datadir / 'drug2ind.txt'
    gene2id = datadir / 'gene2ind.txt'
    fingerprint = datadir / 'drug2fingerprint.txt'

    sample_dir = current_dir / '../sample'

    hidden = sample_dir / 'Hidden_sample'
    result = sample_dir / 'Result_sample'
    batchsize = 100
    ...
    genotype = datadir / 'cell2mutation_2.txt'
    cell_features = alpha * (np.genfromtxt(genotype, delimiter=','))

    CUDA_ID = 0
    predict_dcell(predict_data, num_genes, drug_dim, load, str(hidden), batchsize, str(result), cell_features,
                  drug_features)
